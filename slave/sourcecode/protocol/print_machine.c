...
void FillRAM(uint16 x, uint16 y, uint16 ex, uint16 ey) //x ,ex为8的倍数
{
	...
	//	uint8 n = ex/8;
	SdCmd(0x40);
	SdData(m);//工作窗口左边界位置（第一字符，取值为1）
	SdCmd(0x20);
	SdData(n);//工作窗口右边界位置（水平最大字符数，取值为320）
	SdCmd(0x50);
	SdData(y);//工作窗口上边界位置（第一行，取值为1）
	SdCmd(0x30);
	SdData(ey);//工作窗口下边界位置（垂直最大行，取值为240）

	SdCmd(0x12);
	SdData(0x33);// 同时写入2个显示RAM区域
	SdCmd(0xe0);
	SdData(0x00);// 设置填充数据为00H
	SdCmd(0xf0);
	SdData(0x88);// 选择填充功能，D3=1
	OSTimeDly(10);             // 延迟时间，保证写入时间充分
	SdCmd(0x40);
	SdData(0x00);//工作窗口左边界位置（第一字符，取值为1）
	SdCmd(0x20);
	SdData(0x27);//工作窗口右边界位置（水平最大字符数，取值为320）
	SdCmd(0x50);
	SdData(0x00);//工作窗口上边界位置（第一行，取值为1）
	SdCmd(0x30);
	SdData(0xef);//工作窗口下边界位置（垂直最大行，取值为240）
}
void PrintASCII_LM2088(uint16 x, uint16 y, uint8 *pstr)
//坐标X为屏幕上水平位置，以像素为单位；Y为垂直位置，以像素行为单位
{
	...
	SdData(0x0c);
	SdCmd(0x00);
	SdData(0x0c); // 设置写入数据模式为字符模式
	SdCmd(0x12);
	SdData(0x31); // 打开文本RAM区域（第一显示区域）
	SdCmd(0x60);
	SdData(tmp);    // 设置光标X坐标地址，水平字符数
	SdCmd(0x70);
	SdData(y);    // 设置光标Y坐标地址，垂直像素行
	SdCmd(0xb0);
	while(*pstr > 0)
	{
		delay1us(150);
...
void initLCDM(void)
{
	...
	SdCmd(0x00);
	SdData(0x04);//设置电源为正常模式，软复位无效，写入图形模式，开显示，关闪烁与反显；

	SdCmd(0x0f);
	SdData(0x00);//设置中断使能状态

	SdCmd(0x10);
	SdData(0x00);//设置文本处理方式 光标开，闪烁，跟随字符变化
	SdCmd(0x11);
	SdData(0xe0);//设置光标为块状形式
	SdCmd(0x12);
	SdData(0x31);//设置RAM（显示）工作模式，两个显示区域逻辑或合成显示
	...
	SdCmd(0x01);
	SdData(0x68);//设置BUSY信号高有效；
	SdCmd(0x90);
	SdData(0x6a);//设置数据移位时钟参数
	//----------------------------------------------------------------
	SdCmd(0x21);
	SdData(0x27);//显示窗口右边界位置, 水平最大字符数，取值为320
	SdCmd(0x31);
	SdData(0xef);//显示窗口下边界位置（垂直最大行，取值为240）
	...
	//光标设置
	SdCmd(0x60);
	SdData(0x00);//设置光标位置X坐标，使用初始值
	SdCmd(0x70);
	SdData(0x00);//设置光标位置Y坐标，使用初始值

	SdCmd(0x80);
	SdData(0x33);//设置闪烁时间
	SdCmd(0xf0);
	SdData(0x80);//设置字库，选择GB汉字库和ASCII块0字符库
	SdCmd(0xf1);
	SdData(0x00);//设置字符原大尺寸
	ClearRAM();
}
...
//坐标X为屏幕上水平位置，以像素点为单位；Y为垂直位置，以像素行为单位
{
	uint8 Data, m, i;
	SdCmd(0x00);
	SdData(0x04);  // 设置写入数据模式为图形模式
	SdCmd(0x12);
	SdData(0x32);  // 打开图形RAM区域（第二显示区域）
	SdCmd(0x60);
	SdData(x / 8); // 设置光标X坐标地址，按照字节（含8点）计算
	SdCmd(0x70);
	SdData(y);     // 设置光标Y坐标地址，垂直像素行
	m = x % 8;                // 计算显示位
	Data = 0x80;
...
void PrintReceipt(void)
{
	...
	sprintf(print_buffer, "流水号: %04d%05d%05d", 0,
	        TimeSec(device_control.trade.tm.year + 2000, device_control.trade.tm.month, device_control.trade.tm.day,
